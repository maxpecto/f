<div id="player-wrapper">
    ...
    <div class="ui top attached segment borderless p-0" id="player-embeded" @if($general->site_player == 'embeded') style="display: block;" @else style="display: none;" @endif>
        <div class="ui embed">
            <iframe id="playerEmbeded" src="" allowfullscreen allowtransparency allow="autoplay" poster="/assets/episodes/backdrop/{{ $episode->backdrop }}"></iframe>
        </div>
    ...
    <div class="ui top attached segment borderless p-0" id="player-videojs" @if($general->site_player == 'hls' || $general->site_player == 'direct' || $general->site_player == 'youtube' || $general->site_player == 'trailer') style="display: block;" @else style="display: none;" @endif>
        <video id="playerVideojs" class="video-js vjs-big-play-centered ui embed w-full" controls poster="/assets/episodes/backdrop/{{ $episode->backdrop }}" >
            <p class="vjs-no-js">
    ...
    <div class="sm:flex justify-between items-center">
        <div class="flex space-x-1 w-full p-2 whitespace-nowrap overflow-y-auto pb-4" id="episode-players">
            @if(isset($player['type']))
                @foreach($player['type'] as $key => $value)
                    ...
                    @elseif($value == 'youtube')
                        ...
                        @else
                            <div class="m-2 px-4 py-2 text-white">{{ __("There is no player set for this episode!") }}</div>
                        @endif
                    ...
                    <div class="h-auto p-4 mx-2 text-left bg-gray-900 rounded shadow-xl w-9/12 md:max-w-xl md:p-6 lg:p-8 md:mx-0" @click.away="openReport = false">
                            ...
                            <div class="text-left">
                                ...
                                <div class="mt-6">
                                    ...
                                    <input name="items_id" class="hidden" value="{{ $episode->id }}">
                                    <input name="items_type" class="hidden" value="2">
                                </div>
                            </div>
    ...
    {{-- All Episodes --}}
    <div class="flex justify-start items-center mx-5 py-2 space-x-2 overflow-auto pb-4" >
        @if(!empty($allepisodesforseasons))
            @foreach($allepisodesforseasons as $allepisodes)
            <a href="{{ url($allepisodes->series->id) }}/{{ $allepisodes->series->slug }}/season-{{ $allepisodes->season_id }}/episode-{{ $allepisodes->episode_id}}" class="px-4 py-2 rounded text-white hover:bg-yellow-500 hover:text-white tracking-widest bg-gray-800">E{{ $allepisodes->episode_id }} </a>
            @endforeach
        @endif
    </div>
</div>
...
<script>
    $(function(){
        $('#episode-players a').on('click', function(){
            $('#episode-players a').not(this).removeClass('active');
