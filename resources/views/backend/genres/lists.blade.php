...
<div class="w-full p-5 text-white bg-gray-900 rounded-t flex justify-between">
    <span>Genres</span><span>Total Genres : {{ count($total_genres)  }}</span>
</div>
<section class="w-full text-white rounded-b">
    ...
    <div class="flex bg-gray-800 justify-between items=center p-5">
        ...
        <div class="flex space-x-2 items-center">
            ...
            <div x-cloak x-data="{ open: false }">
                <button class="px-4 py-2 bg-green-500 uppercase text-white rounded focus:outline-none flex items-center" @click="open = true"><span class="iconify mr-1" data-icon="gridicons:create" data-inline="false"></span> Create Tags</button>
                <div class="fixed z-50  top-0 left-0 flex items-center justify-center w-full h-full" style="background-color: rgba(0,0,0,.5);" x-show="open" >
                    <div class="h-auto p-4 mx-2 text-left bg-gray-900 rounded shadow-xl w-9/12 md:max-w-xl md:p-6 lg:p-8 md:mx-0" @click.away="open = false">
                        {!! Form::open(['url' => 'admin/genres/store','method'=>'post']) !!}
                            <div class="text-center">
                                ...
                                <div class="mt-6">
                                    <p class="text-sm leading-5 text-gray-500">
                                        <input type="text" name="name" autofocus class="w-full p-2 bg-gray-200 text-gray-500 rounded border border-transparent focus:outline-none focus:ring-2 focus:ring-green-600 focus:border-transparent" placeholder="Tag Name..">
                                    </p>
                                </div>
                            </div>
    ...
    <div class="bg-gray-700 justify-between items=center border-1 border-t border-gray-700 p-5 overflow-x-scroll">
        <div>
            {!! Form::open(['route' => ['admin.genres'] ,'method' => 'get' ,'class' => 'flex items-center mb-5 w-full relative']) !!}
        ...
        <table class="min-w-full leading-normal">
            <thead>
                <tr>
                    ...
                    <th class="px-5 py-3 border-b-2 border-gray-200 bg-gray-900 text-left text-xs font-semibold text-white uppercase tracking-wider text-left">Name</th>
                            ...
                            <td class="px-5 py-5 border-b border-gray-200 bg-white text-sm text-right">
                                <div class="flex space-x-2 items-center float-right">
                                    ...
                                    <div x-cloak x-data="{ open: false }">
                                        ...
                                        <div class="fixed z-50  top-0 left-0 flex items-center justify-center w-full h-full" style="background-color: rgba(0,0,0,.5);" x-show="open" >
                                            <div class="h-auto p-4 mx-2 text-left bg-gray-900 rounded shadow-xl w-9/12 md:max-w-xl md:p-6 lg:p-8 md:mx-0" @click.away="open = false">
                                                {!! Form::model($row, ['route' => ['admin.genres-update', $row->id ], 'method'=>'put' ]) !!}
                                                    <div class="text-center">
                                                        <h3 class="text-lg font-medium leading-6 text-white uppercase border-1 border-b border-gray-600 w-max pb-4">
                                                            Update Genres
                                                        </h3>
                                                        <div class="mt-6">
                                                            <p class="text-sm leading-5 text-gray-500">
                                                                <input type="text" name="name" autofocus value="{{ $row->name }}" class="w-full p-2 bg-gray-200 text-gray-500 rounded border border-transparent focus:outline-none focus:ring-2 focus:ring-green-600 focus:border-transparent" placeholder="Tag Name..">
                                                            </p>
                                                        </div>
                                                    </div>
                                                    <div class="mt-5 sm:mt-6">
                                    ...
                                    <!-- Modal end -->
                                    <a href="{{ url('/admin/genres/delete/['.$row->id.']') }}" class="px-4 py-2 bg-red-500 uppercase text-white rounded flex items-center"><span class="iconify mr-1" data-icon="fluent:delete-dismiss-28-filled" data-inline="false"></span> Delete</a>
                                </div>
                            </td>
                        </tr>
                    @endforeach
                @else
                    <tr><td colspan="5" class="px-5 py-5 border-b border-gray-200 bg-white text-sm text-center"><div class="ui orange message"><b>You dont have any genres</b></div></td></tr>
                @endif
            </tbody>
            <tfoot>
				<tr>
...
<script type="text/javascript">
    $('.basic.checkbox input').on('change', function() {
        ...
        $('#delete')[0].href = '/admin/genres/delete/' + postsId;
    });
    ...
    $('.visible.checkbox input').on('change', function() {
        ...
        if(/^(0|1)$/.test(visible) && /^(\d+)$/.test(id))
        {
            ...
            var ajaxReqUrl  = '/admin/genres/visible';
