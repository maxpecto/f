...
<div class="w-full p-5 text-white bg-gray-900 rounded-t flex justify-between">
    <span>Pages</span><span>Total Pages : {{ count($total_pages)  }}</span>
</div>
<section class="w-full text-white rounded-b">
    ...
    <div class="flex bg-gray-800 justify-between items=center p-5">
        ...
        <div class="flex space-x-2 items-center">
            <a href="/admin/pages/add" class="px-4 py-2 bg-green-500 uppercase text-white rounded focus:outline-none flex items-center"><span class="iconify mr-1" data-icon="gridicons:create" data-inline="false"></span> Add Page</a>
        </div>
    </div>

    <div class="bg-gray-700 justify-between items=center border-1 border-t border-gray-700 p-5 overflow-x-scroll">
        <div>
            {!! Form::open(['route' => ['admin.pages'] ,'method' => 'get' ,'class' => 'flex items-center mb-5 w-full relative']) !!}
        ...
        <table class="min-w-full leading-normal">
            <thead>
                <tr>
                    ...
                    <th class="px-5 py-3 border-b-2 border-gray-200 bg-gray-900 text-left text-xs font-semibold text-white uppercase tracking-wider text-left">Page Title</th>
                    <th class="px-5 py-3 border-b-2 border-gray-200 bg-gray-900 text-left text-xs font-semibold text-white uppercase tracking-wider text-center">Views</th>
                ...
                @if(!$data->isEmpty())
                    @foreach($data as $i=>$row)
                        <tr>
                            ...
                            <td class="px-5 py-5 border-b border-gray-200 bg-white text-sm text-center leading-9">
                            ...
                            <td class="px-5 py-5 border-b border-gray-200 bg-white text-sm text-right">
                                <div class="flex space-x-2 items-center float-right">
                                    <a href="{{ url('/admin/pages/edit/'.$row->id) }}" class="px-4 py-2 bg-green-500 uppercase text-white rounded focus:outline-none flex items-center"><span class="iconify mr-1" data-icon="akar-icons:edit" data-inline="false"></span> Edit</button>

                                    <a href="{{ url('/admin/pages/delete/['.$row->id.']') }}" class="px-4 py-2 bg-red-500 uppercase text-white rounded flex items-center"><span class="iconify mr-1" data-icon="fluent:delete-dismiss-28-filled" data-inline="false"></span> Delete</a>
                                </div>
                            </td>
                        </tr>
                    @endforeach
...
<script type="text/javascript">
    $('.basic.checkbox input').on('change', function() {
        ...
        $('#delete')[0].href = '/admin/pages/delete/' + postsId;
    });
