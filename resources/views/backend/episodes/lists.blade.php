...
<div class="w-full p-5 text-white bg-gray-900 rounded-t flex justify-between">
    <span>Episodes</span><span>Total Episodes : {{ count($total_episodes)  }}</span>
</div>
<section class="w-full text-white rounded-b">
    ...
    <div class="flex bg-gray-800 justify-between items=center p-5">
        ...
        <div class="flex space-x-2 items-center">
            <a href="/admin/episodes/add" class="px-4 py-2 bg-green-500 uppercase text-white rounded focus:outline-none flex items-center"><span class="iconify mr-1" data-icon="gridicons:create" data-inline="false"></span> Add Episodes</a>
        </div>
    </div>

    <div class="bg-gray-700 justify-between items=center border-1 border-t border-gray-700 p-5 overflow-x-scroll">
        <div>
            {!! Form::open(['route' => ['admin.episodes'] ,'method' => 'get' ,'class' => 'flex items-center mb-5 w-full relative']) !!}
        ...
        <table class="min-w-full leading-normal">
            <thead>
                <tr>
                    ...
                    <th class="px-5 py-3 border-b-2 border-gray-200 bg-gray-900 text-left text-xs font-semibold text-white uppercase tracking-wider text-left">Series Episodes</th>
                    <th class="px-5 py-3 border-b-2 border-gray-200 bg-gray-900 text-left text-xs font-semibold text-white uppercase tracking-wider text-left">Episode Name</th>
                ...
                @if(!$data->isEmpty())
                    @foreach($data as $i=>$row)
                        <tr>
                            ...
                            <td class="px-5 py-5 border-b border-gray-200 bg-white text-sm text-cenleftter w-80">
                                <div>
                                    <div class="flex items-center">
                                        <img class="h-10 md:h-20 mr-5 hidden xl:block rounded" src="/assets/episodes/backdrop/episodes_image_{{ $row->id }}.jpg"/>
                                        <div class="flex-wrap">
                                            <strong class="flex overflow-hidden">{{ $row->series->title }} (S{{ $row->season_id }}E{{ $row->episode_id }})</strong>
                                            <div class="text-xs mt-2 mb-2"><strong>Air Date</strong> {{ $row->air_date }}</div>
                                            <div class="text-xs mt-2 mb-2 flex items-center space-x-2"><strong class="flex items-center space-x-1"> <span class="iconify" data-icon="bx:bxs-like" data-inline="false"></span><div>{{ totalEpisodeLike($row->id) }}</div></strong>  <strong class="flex items-center space-x-1"><span class="iconify" data-icon="bx:bxs-dislike" data-inline="false"></span><div>{{ totalEpisodeDislike($row->id) }}</div></strong> </div>
                                        </div>
                                    </div>
                                <div>
                            </td>
                            <td class="px-5 py-5 border-b border-gray-200 bg-white text-sm text-left">
                                <div class="flex-wrap items-center">
                                    ...
                                    <p class="text-xs hidden xl:block">{{ $row->description }}</p>
                                </div>
                            </td>
                            <td class="px-5 py-5 border-b border-gray-200 bg-white text-sm text-center">
                            ...
                            <td class="px-5 py-5 border-b border-gray-200 bg-white text-sm text-right">
                                <div class="flex space-x-2 items-center float-right">
                                    <a href="{{ url('/admin/episodes/edit/'.$row->id) }}" class="px-4 py-2 bg-green-500 uppercase text-white rounded focus:outline-none flex items-center"><span class="iconify mr-1" data-icon="akar-icons:edit" data-inline="false"></span></button>

                                    <a href="{{ url('/admin/episodes/delete/['.$row->id.']') }}" class="px-4 py-2 bg-red-500 uppercase text-white rounded flex items-center"><span class="iconify mr-1" data-icon="fluent:delete-dismiss-28-filled" data-inline="false"></span></a>
                                </div>
                            </td>
                        </tr>
                    @endforeach
