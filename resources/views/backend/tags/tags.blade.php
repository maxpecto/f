...
<div class="w-full p-5 text-white bg-gray-900 rounded-t flex justify-between">
    <span>Tags</span><span>Total Tags : {{ count($total_tags)  }}</span>
</div>
<section class="w-full text-white rounded-b">
            ...
            <div x-cloak x-data="{ open: false }">
                ...
                <div class="fixed z-50  top-0 left-0 flex items-center justify-center w-full h-full" style="background-color: rgba(0,0,0,.5);" x-show="open" >
                    <div class="h-auto p-4 mx-2 text-left bg-gray-900 rounded shadow-xl w-9/12 md:max-w-xl md:p-6 lg:p-8 md:mx-0" @click.away="open = false">
                        {!! Form::open(['url' => 'admin/tags/store','method'=>'post']) !!}
    ...
    <div class="bg-gray-700 justify-between items=center border-1 border-t border-gray-700 p-5 overflow-x-scroll">
        <div>
            {!! Form::open(['route' => ['tags'] ,'method' => 'get' ,'class' => 'flex items-center mb-5 w-full relative']) !!}
        ...
        <table class="min-w-full leading-normal">
            <thead>
                <tr>
                    ...
                    <th class="px-5 py-3 border-b-2 border-gray-200 bg-gray-900 text-left text-xs font-semibold text-white uppercase tracking-wider text-left">Tag Name</th>
                            ...
                            <td class="px-5 py-5 border-b border-gray-200 bg-white text-sm text-right">
                                <div class="flex space-x-2 items-center float-right">
                                    ...
                                    <div x-cloak x-data="{ open: false }">
                                        ...
                                        <div class="fixed z-50  top-0 left-0 flex items-center justify-center w-full h-full" style="background-color: rgba(0,0,0,.5);" x-show="open" >
                                            <div class="h-auto p-4 mx-2 text-left bg-gray-900 rounded shadow-xl w-9/12 md:max-w-xl md:p-6 lg:p-8 md:mx-0" @click.away="open = false">
                                                {!! Form::model($row, ['route' => ['tags-update', $row->id ], 'method'=>'put' ]) !!}
                                                    <div class="text-center">
                                                        <h3 class="text-lg font-medium leading-6 text-white uppercase border-1 border-b border-gray-600 w-max pb-4">
                                                            Update Tag
                                                        </h3>
                                    ...
                                    <!-- Modal end -->
                                    <a href="{{ url('/admin/tags/delete/['.$row->id.']') }}" class="px-4 py-2 bg-red-500 uppercase text-white rounded flex items-center"><span class="iconify mr-1" data-icon="fluent:delete-dismiss-28-filled" data-inline="false"></span> Delete</a>
                                </div>
                            </td>
                        </tr>
                    @endforeach
                @else
                    <tr><td colspan="5" class="px-5 py-5 border-b border-gray-200 bg-white text-sm text-center"><div class="ui orange message"><b>You dont have any tags</b></div></td></tr>
                @endif
            </tbody>
            <tfoot>
				<tr>
