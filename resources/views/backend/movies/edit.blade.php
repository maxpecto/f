...
<section class="w-full text-white rounded-b">
    {!! Form::model($movies, ['route' => ['admin.movies-update',$movies->id],'method'=>'put','enctype' => 'multipart/form-data','class' => 'form movie']) !!}
    <div class="bg-gray-700 justify-between items=center border-1 border-t border-gray-700 p-5 w-full xl:flex space-x-0 xl:space-x-6 space-y-6 xl:space-y-0">
        <div class="w-full xl:w-3/12 space-y-6">
            <div class="w-full" id="tmdb">
                ...
                <div class="w-full">
                    <input class="w-full rounded-t p-2 bg-gray-600 text-white disabled" type="text" name="movie_id" placeholder="TMDB ID" value="{{ $movies->tmdb_id }}">
                </div>
            ...
            <div class="w-full field image">
                <img src="{{  asset('/assets/movies/poster/'.$movies->poster) }}" alt="poster image" id="movie_poster" class="w-full">
                <input type="hidden" name="movie_poster_url" value="{{  asset('/assets/movies/poster/'.$movies->poster) }}">
            ...
            <div class="w-full field image">
                <img src="{{  asset('/assets/movies/backdrop/'.$movies->backdrop) }}" alt="backdrop image" id="movie_image" class="w-full">
                <input type="hidden" name="movie_image_url" value="{{  asset('/assets/movies/backdrop/'.$movies->backdrop) }}">
                ...
                <div class="w-full">
                    <select name="platform_id" id="platform_id" class="w-full rounded p-2 bg-gray-600 text-white select2-basic">
                        ...
                        @foreach ($platforms as $platform)
                            <option value="{{ $platform->id }}" {{ (old('platform_id', $movies->platform_id) == $platform->id) ? 'selected' : '' }}>
                                {{ $platform->name }}
                            </option>
                        @endforeach
                    </select>
            ...
            <div class="w-full">
                ...
                <div class="w-full">
                    <input name="movie_duration" type="text" placeholder="Duration" value="{{ $movies->duration }}" class="w-full rounded p-2 bg-gray-600 text-white">
                </div>
            </div>
            <div class="w-full">
                <label><h4 class="w-full mb-2 font-medium">Rating</h4></label>
                <div class="w-full">
                    <input name="movie_rating" type="text" placeholder="Rating" value="{{ $movies->rating }}" class="w-full rounded p-2 bg-gray-600 text-white">
                </div>
            </div>
            <div class="w-full">
                <label><h4 class="w-full mb-2 font-medium">Release Date (YYYY-MM-DD)</h4></label>
                  <div class="w-full">
                    <input name="movie_release_date" type="text" placeholder="Release Date" value="{{ $movies->release_date }}" class="w-full rounded p-2 bg-gray-600 text-white">
                </div>
            </div>
        </div>
        <div class="w-full xl:w-9/12 space-y-6">
            <div class="w-full">
                <label><h4 class="w-full mb-2 font-medium">Name</h4></label>
                  <div class="control">
                    <input name="movie_name" id="movie_name" type="text" placeholder="Name" value="{{ $movies->title }}" class="w-full rounded p-2 bg-gray-600 text-white">
                </div>
            </div>
            <div class="w-full">
                <label><h4 class="w-full mb-2 font-medium">Description</h4></label>
                <div class="control">
                    <textarea name="movie_description" placeholder="Description" class="w-full rounded p-4 h-20 bg-gray-600 text-white leading-6">{{ $movies->overviews }}</textarea>
                </div>
            </div>
            <div class="w-full">
            ...
            <div class="w-full">
                <label><h4 class="w-full mb-2 font-medium">Trailer</h4></label>
                  <div class="control">
                    <input name="movie_trailer" type="text" placeholder="Trailer Url.." class="w-full rounded p-2 bg-gray-600 text-white" value="{{ $movies->trailer }}">
                </div>
            </div>
            <div class="w-full">
...
<script>
	...
	var takenactor = {!! $movies->actors->pluck('tmdb_id') !!};
	var takendirector = {!! $movies->directors->pluck('tmdb_id') !!};
	var takenwriter = {!! $movies->writers->pluck('tmdb_id') !!};
