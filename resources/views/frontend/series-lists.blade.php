                ...
                <div>
                    <div class="ui text menu m-0 flex justify-between items-center pb-4 text-yellow-400">
                        <div class="flex items-center space-x-1">
                            <span class="iconify" data-icon="ic:baseline-splitscreen" data-inline="false"></span> <a class="item text-xl  font-medium">{{ __("Latest Series") }}</a>
                        </div>
                        <div>
                    ...
                    <div id="myFilter" class="transition delay-150 duration-300 ease-in-out" style="display:none">
                        {!! Form::open(['url' => 'series','method'=>'get','class'=>'w-full grid xl:grid-cols-6 lg:grid-cols-3 md:grid-cols-3 sm:grid-cols-2 grid-cols-1 mb-4 text-xs']) !!}
                    ...
                    <div class="w-full grid xl:grid-cols-5 lg:grid-cols-4 md:grid-cols-3 sm:grid-cols-2 grid-cols-1">
                        @if(count($series) >= 1)
                            ...
                            @endforeach
                        @else
                            <div class="flex items-center text-yellow-400"><span>{{ __("There is no latest series!") }}</span></div>
                        @endif
                    </div>
                    {{-- Pagination --}}
                    <div class="flex mt-5">
                        {{ $series->links('frontend.layouts.pagination')  }}
                    </div>
